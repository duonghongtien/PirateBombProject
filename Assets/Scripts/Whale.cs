...
public class Whale : MonoBehaviour
{
    ...
    public float dialogBoxHeightOffsety = 1f; // Khoảng cách offset theo trục Y
    public float dialogBoxHeightOffsetx = -1f; // Khoảng cách offset theo trục Y
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < khoangcachplayer)
        {
            ...
            else
            {
                if (!isAttacking)
                {
                    animator.SetInteger("chayWhale", 1);
                ...
                {
                    isAttacking = true;
                    animator.SetTrigger("tancongplayer");
                }
            }
        }
        ...
        {
            animator.SetInteger("chayWhale", 0);
            if (isChasing)
            {
                animator.SetInteger("chayWhale", 0);
                isChasing = false;
    ...
    private void SetDialogBoxPosition()
    {
        ...
        dialogBoxPosition.y += dialogBoxHeightOffsety;
        dialogBoxPosition.x += dialogBoxHeightOffsetx;
